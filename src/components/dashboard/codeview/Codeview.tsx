...
import { Framework, type Project } from "@/types";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
...
function CodeviewInner({ isSaving, onSendErrorToChat }: CodeviewProps) {
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          await axios.patch(
            `${API_CONFIG.BACKEND_URL}/projects/${activeProject.projectId}/codebase?walletAddress=${address}`,
            payload
          );
  ...
  const fetchVersionBasedCode = async (versionId: number) => {
    try {
      setIsLoading(true);
      toast.info("Loading code version...");

      const response = await axios.get(
        `${API_CONFIG.BACKEND_URL}/projects/${activeProject?.projectId}/versions/${versionId}?walletAddress=${address}`
      );
      ...
      {isCodeGenerating ? (
        <div className="flex items-center justify-center h-full">
          {(() => {
            ...
            return (
              <img src={memeSrc} height={280} width={280} alt="loading_gif" />
            );
          })()}
        </div>
      ) : (
                      ...
                      {codeVersions.length > 1 &&
                        codeVersions.slice(1).map((version) => (
                          <button
                            key={version.id}
                            onClick={() => {
                              fetchVersionBasedCode(version.id);
                              setIsVersionDropdownOpen(false);
                            }}
                            className={cn(
